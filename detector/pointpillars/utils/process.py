...
""" def match_boxes(preds, gts, iou_thresh=0.5):
    ...
    for pred in sorted(preds, key=lambda x: -x['score']):
        max_iou, matched = 0.0, -1
        for i, gt in enumerate(gts):
            if i in matched_gt or pred['name'] != gt['name']:
                continue
            iou = iou2d(pred['translation'] + pred['size'] + [pred['rotation_y']],
                                 gt['translation'] + gt['size'] + [gt['rotation_y']])
            if iou > max_iou:
                max_iou, matched = iou, i
...
def match_boxes(preds, gts, iou_thresh=0.5):
    ...
    for pred in sorted(preds, key=lambda x: -x['score']):
        max_iou, matched = 0.0, -1
        pred_box = torch.tensor(pred['translation'] + pred['size'] + [pred['rotation_y']])

        for i, gt in enumerate(gts):
            if i in matched_gt or pred['name'] != gt['name']:
                continue
            gt_box = torch.tensor(gt['translation'] + gt['size'] + [gt['rotation_y']])

            iou = iou2d_nearest(pred_box.unsqueeze(0), gt_box.unsqueeze(0))[0, 0]  # 取出标量
            if iou > max_iou:
                max_iou, matched = iou, i
...
def compute_ap(tp, fp, scores, num_gt):
    ...
    for r in recall_levels:
        ...
        ap += max(precisions) if len(precisions) > 0 else 0.0
    return ap / 11
